...
def segment_chars(plate_img, fixed_width): 
	...
	""" 
	extract Value channel from the mp4 format 
...
if __name__ == "__main__": 
    ...
    model = OCR(modelFile="D:/Python code/Machine Learning/Plate Capturing/binary_128_0.50_ver3.pb",
                labelFile="D:/Python code/Machine Learning/Plate Capturing/binary_128_0.50_labels_ver2.txt")

    cap = cv2.VideoCapture('test.mp4') 

    # Open the CSV file for writing
    with open('dat.csv', mode='w', newline='') as csv_file:
        fieldnames = ['PlateCharacters']
        writer = csv.DictWriter(csv_file, fieldnames)
        writer.writeheader()

        # Create the output directory if it doesn't exist
        if not os.path.exists('D:\Python code\Machine Learning\Plate Capturing\output'):
            os.makedirs('D:\Python code\Machine Learning\Plate Capturing\output')
            ...
            if ret == True: 
                ...
                if possible_plates is not None: 
                    for i, p in enumerate(possible_plates): 
                        ...
                        recognized_plate = model.label_image_list(chars_on_plate, imageSizeOuput=128) 
                        print(recognized_plate)

                        # Save the plate image
                        output_image_path = f'D:\\Python code\\Machine Learning\\Plate Capturing\\output_{frame_count}_{i}.png'
                        cv2.imwrite(output_image_path, p) 

                        # Write to CSV
                        writer.writerow({'PlateCharacters': recognized_plate})
